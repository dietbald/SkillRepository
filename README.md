...
| `ph-supplier-enrich` | Enrich a list of Philippine supplier short-names with authoritative registration data. Runs three sources in waterfall per supplier: DuckDuckGo (candidate names + contacts), DTI BNRS (sole-prop registrations), SEC company search (corporations). Resumable; rate-limited. Use when you have an informal canvas/RFQ supplier list and want full enrichment. |
| `dti-bnrs-lookup` | DTI BNRS — Philippine sole-proprietorship registry search. Single name or batch CSV. Returns business_name, owner, cert_no, registration_date, status, territory, scope. Standalone version of the DTI module from `ph-supplier-enrich`; use when you only need DTI (faster, fewer captcha solves). Needs the `2captcha-cli` API key. |
| `philgeps-merchant` | PhilGEPS — Philippine Government Electronic Procurement System merchant registry. Single keyword or batch CSV. Returns registered legal name, type (Corporation / Sole Proprietorship / Partnership), certification level (Platinum/Gold/Silver/Bronze/Red), PhilGEPS reg#, Mayor's Permit, Tax Clearance, SEC#. Use to verify accreditation status and pull authoritative legal names. |
